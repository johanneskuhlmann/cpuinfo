...
inline static uint32_t parse_processor_number(
...
inline static void parse_cpu_architecture(
...
inline static const uint32_t uintpow(const uint32_t base, const uint32_t exponent){
	uint32_t result=1;
...
inline static const uint32_t parse_cpu_clock(
	const char *clock_start,
	const char *clock_end)
{
	const size_t clock_length = (size_t) (clock_end - clock_start);

	if (clock_length == 0) {
		cpuinfo_log_warning("Clock in /proc/cpuinfo is ignored: string is empty");
		return 0;
	}

	uint32_t clock = 0; // in khz
	const char* parsed_end = parse_number( clock_start, clock_end, &clock);
	if (parsed_end == clock_start || parsed_end == clock_end) {
		cpuinfo_log_warning("Clock in /proc/cpuinfo is ignored: NUMBER (Hz, KHz, MHz, GHz) is not specified");
		return 0;
	}

	// should have stopped at "." separator
	uint32_t rhs = 0;
	const char* dot_ptr = (*parsed_end == '.')?parsed_end:clock_start;
	const char* range_ptr = (*parsed_end == '.')?parse_number( dot_ptr+1, clock_end, &rhs):clock_start;
	const size_t rhs_length = (range_ptr > dot_ptr)?(size_t)( range_ptr - (dot_ptr+1) ):0;

	// read range specifier (Hz, KHz, MHz, GHz)
	if (memcmp(range_ptr, "Hz", 2) == 0) {
		return clock/1000;
	} else if (memcmp(range_ptr, "KHz", 3) == 0) {
		return clock;
	} else if (memcmp(range_ptr, "MHz", 3) == 0) {
		return clock*1000 + rhs/uintpow(10,rhs_length-3);
	} else if (memcmp(range_ptr, "GHz", 3) == 0) {
		return clock*1000000 + rhs/uintpow(10,rhs_length-6);
	} else {
		cpuinfo_log_warning("Clock in /proc/cpuinfo is ignored: range specifier not in (Hz, KHz, MHz, GHz)");
		return 0;
	}
}

inline static void parse_cpu_pvr(
	...
	switch (key_length) {
		...
		case 5:
			if (memcmp(line_start, "clock", key_length) == 0) {
				processor->max_frequency = parse_cpu_clock(value_start, value_end);
				processor->min_frequency = processor->max_frequency;
			} else {
				goto unknown;
			}
			break;
		...
		case 9:
			if (memcmp(line_start, "processor", key_length) == 0) {
				...
				processors[state->processor_index].flags |= CPUINFO_LINUX_FLAG_VALID;
				return true;
			} else {
				goto unknown;
			}
